...
def fingerprints(arr, winSize = 4):
    ...
    for i in range(arrLen - winSize):
        ...
        if not currMin == prevMin:  #min value of window is stored only if it is not the same as min value of prev window
            ...
            prevMin = currMin  #density of winnowing and guarantee threshold (Stanford paper)
...
def plagiarismCheck(file1 , text1, file2, text2):
    f1 = open(file1, "r")
    token1 = tokenize(file1, text1)  
    str1 = toText(token1)
    token2 = tokenize(file2, text2)
    str2 = toText(token2)
    ...
    startx = []   #to store the start values corresponding to matching fingerprints
    endx = []   #to store end values
